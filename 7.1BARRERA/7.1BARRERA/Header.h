...
//1. primer multiplo de 7 que no está en camino de primos
//2. profundidad a al que te encuentras
std::pair<int, int> barrera(const BinTree<int>& tree) {
    if (tree.empty()) {
        return { -1, -1 };
    }/*
     ...
     */
        ...
        if (esPrimo(tree.root())) {
            ...
            auto [mul_izq, prof_izq] = barrera(tree.left());
            auto [mul_der, prof_der] = barrera(tree.right());
            if (prof_izq == -1 && prof_der != -1) {
                return{ mul_der, prof_der + 1 };
            }
            else if (prof_izq != -1 && prof_der == -1) {
                return{ mul_izq, prof_izq + 1 };
            }
            else if (prof_izq != -1 && prof_der != -1) {
                if (prof_izq <= prof_der) {
                    return{ mul_izq, prof_izq + 1 };
                }
                else {//prof_izq > prof_der
                    return{ mul_der, prof_der + 1 };
                }
            }
            else {
...
std::pair<int, int> barrera_enAnchura(const BinTree<int>& tree) {
    queue<pair<int, BinTree<int>>> pending;
    if (tree.empty()) {
        return{ -1,-1 };
    }
    else {
        ...
        pending.push({ 1, tree });
        while (!pending.empty()) {
            auto [prof_current, current] = pending.front();
            pending.pop();
            if (current.root() != 7 && current.root() % 7 == 0) {
                found = true;
            }
            else if (esPrimo(current.root())) {
                //no hacer nada
            }
            else {//no es ni primo, ni tampoco multiplo de 7
                if (!tree.left().empty()) {
                    pending.push({ prof_current + 1,tree.left() });
                }
                if (!tree.right().empty()) {
                    pending.push({ prof_current + 1,tree.right() });
                }
