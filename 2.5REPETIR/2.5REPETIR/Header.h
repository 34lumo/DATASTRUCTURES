...
void ListLinkedSingle:: replicate(const ListLinkedSingle& ys) {
    Node* yscurrent = ys.head;
    Node* current = this->head;
    Node* previo = nullptr;
    Node* aux = nullptr;

    while (yscurrent != nullptr) {
        if (yscurrent->value == 0) {//eliminar
        ...
        else if (yscurrent->value == 1) {//dejar igual
            
        }
        else if (yscurrent->value > 1) {//duplicar n-1 veces
