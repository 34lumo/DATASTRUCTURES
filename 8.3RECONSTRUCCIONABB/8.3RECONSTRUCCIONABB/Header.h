...
BinTree<int> reconstruccion(std:: vector<int>& v, int ini, int fin) {
    if (ini == fin) {
        return BinTree<int>();
    }
    else {
        ...
        while (ini + cont < fin && v[ini + cont] <= root) {
            cont++;
        }
        left = reconstruccion(v, ini+1, ini+cont);
        right = reconstruccion(v, ini+cont, fin);
